...
class HereRoutingConnector(BaseConnector):
    ...
    def _call_routes(self, waypoints: List[LatLng], opts: RoutingOptions) -> RoutingResult:
        ...
        if not routes:
            raise unknown_error(resp.status, raw, "HERE Routing returned no routes")
        route = routes[0]
        ...
        for section in route.get("sections") or []:
            ...
            legs.append(RoutingLeg(length, duration))
            total_dist += length
            total_dur += duration

        return RoutingResult(legs=legs, total_distance_meters=total_dist, total_duration_seconds=total_dur, polyline=encode_polyline(all_coords), raw=raw)
    ...
    def _find_sequence(self, opts: RoutingOptions) -> List[int]:
        ...
        query = {
            ...
            "mode": f"fastest;{tm};traffic:disabled",
        }
        if opts.departure_time:
...
class HereGeocodingConnector(BaseConnector):
    ...
    def geocode(self, opts: GeocodeOptions) -> GeocodeResult:
        ...
        if opts.country_filter:
            alpha3 = []
            for code in opts.country_filter:
                if not code.strip():
                    continue
            ...
            if alpha3:
                base_query["in"] = "countryCode:" + ",".join(alpha3)
    ...
    def autocomplete(self, opts: AutocompleteOptions) -> AutocompleteResult:
        ...
        raw = self._get(_AUTOSUGGEST_URL, base_query, opts.passthrough)
        preds = [AutocompletePrediction(description=jstr(jget(it, "title")), place_id=jstr(jget(it, "id")) or None) for it in (jlist(jget(raw, "items")) or [])]
...
def _normalize_here_items(raw: Any) -> List[GeocodeCandidate]:
    ...
    for item in jlist(jget(raw, "items")) or []:
        ...
        out.append(GeocodeCandidate(formatted_address=addr, location=LatLng(jnum(pos.get("lat")), jnum(pos.get("lng"))), place_id=jstr(jget(item, "id")) or None, viewport=viewport))
    return out
